...
# https://stackoverflow.com/a/17287984/4156752
# extract proto/scheme
driver="`echo $DATABASE_URL | grep '://' | sed -e's|^\(.*\)://.*|\1|g'`"
...
database="`echo $url | grep / | cut -d/ -f2- | cut -d? -f1`"
...
# https://stackoverflow.com/a/70362046/4156752
start_time="$(date -u +%s)"
...
if [[ $driver == 'mysql' ]]; then
  until [ $elapsed_seconds -gt $DEPS_TIMEOUT ] || mysql --host="$host" --port="$port" --user="$user" --password="$password" -e "SELECT 1" >/dev/null 2>&1; do
    current_time="$(date -u +%s)"
    elapsed_seconds=$(($current_time - $start_time))
    entrypoint_warn "Still waiting for db to be ready... Or maybe the db is not reachable. $(($elapsed_seconds - $DEPS_TIMEOUT)) seconds left"
    sleep 1
  done
